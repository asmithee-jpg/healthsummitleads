...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const leads = (await db.getLeads()).filter(l => !vendorId || l.vendorId === vendorId)

  const rows = [
    ['Name', 'Title', 'Organization', 'Email', 'Phone', 'Interest', 'Note', 'Captured At'],
    ...leads.map(l => [
      l.attendeeName, l.attendeeTitle, l.attendeeOrg, l.attendeeEmail,
      l.attendeePhone || '', l.interest, l.note,
